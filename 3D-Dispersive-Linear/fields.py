...
def TIME_SOURCE(polarization,n_f,del_t,n_t,wavelength,fwhm,del_l,location,injection_axis,injection_direction):
    ...
    #computer fwhm in time
    fwhm = fwhm/(2*c0)
    #standard deviation
    sigma = fwhm / 2.35482
    #electric field time source
    e_time_source = np.sin(-omega*t)*np.exp(-(t-2*sigma)**2/(2*sigma**2))

    #current density required to produce a plane wave with the given electric field time source
    current_density = -2*e_time_source/eta_0

    #initilize a time source
    time_source = np.zeros((n_t,n_f),dtype = float)
    ...
    return time_source , e_time_source
...
def SOURCE(n_f,n_t,del_t,del_l,n_x,n_y,n_z,polarization,wavelength,fwhm,location,injection_axis,injection_direction,source_type,fwhm_mode,n_m,center_mode,mode_axis):
    ...
    time_source , e_time_source = TIME_SOURCE(polarization,n_f,del_t,n_t,wavelength,fwhm,del_l,location,injection_axis,injection_direction)
    
    if source_type == 'Line':

        space_time_source = LINE_SOURCE(location,injection_axis,time_source,n_x,n_y,n_z)

        return space_time_source , time_source , e_time_source
